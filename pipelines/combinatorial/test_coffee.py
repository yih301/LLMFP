...
The task is: A coffee production company sources beans from three suppliers, roasts them at one of two facilities into either dark or light coffee, and ships the roasted coffee to three retail locations. Each supplier has a limited capacity. Each roastery, with no existing inventory, can roast one unit coffee bean into one unit of dark or light coffee. The retail locations have specific demands for dark and light coffee, with no existing inventory. The company's objective is to minimize the total cost, including shipping beans, roasting, and shipping roasted coffee, while ensuring that all coffee produced meets or exceeds the demand at each retail location.
You have the access to variables that summarized the original supplier, roastery, and retail information: 
...
The task is: A coffee production company sources beans from three suppliers, roasts them at one of two facilities into either dark or light coffee, and ships the roasted coffee to three retail locations. Each supplier has a limited capacity and each roastery can roast one unit coffee bean into one unit of dark or light coffee. The retail locations have specific demands for dark and light coffee, with no existing inventory. The company's objective is to minimize the total cost, including shipping beans, roasting, and shipping roasted coffee, while ensuring that all coffee produced meets or exceeds the demand at each retail location.
Query: {{{question}}}
...
Query: {{{question}}}
Variable or API: 
You have the access to variables that summarized the ORIGINAL supplier capacity and retail demand information: 
capacity_in_supplier = {'supplier1': 150, 'supplier2': 50, 'supplier3': 100}
dark_coffee_needed_for_cafe = {'cafe1': 20, 'cafe2': 20, 'cafe3': 100}
light_coffee_needed_for_cafe = {'cafe1': 20, 'cafe2': 30, 'cafe3': 40}
...
{
    "suppliers": {
        "supplier_1_ship_to_roastery_1": ,
        "supplier_1_ship_to_roastery_2": ,
        "supplier_2_ship_to_roastery_1": ,
        "supplier_2_ship_to_roastery_2": , 
        "supplier_3_ship_to_roastery_1": ,
        "supplier_3_ship_to_roastery_2": ,
        "supplier_1_total_supplied_beans": ,
        "supplier_2_total_supplied_beans": , 
        "supplier_3_total_supplied_beans": ,
        },
    "roasteries": {
        "roastery_1_dark_coffee": , 
        "roastery_1_light_coffee": , 
        "roastery_2_dark_coffee": , 
        "roastery_2_light_coffee": , 
        "roastery_1_total_roasted_beans": ,
        "roastery_2_total_roasted_beans": ,
        },
    "cafe": {
        "roastery_1_ship_dark_to_cafe_1": ,
        "roastery_1_ship_light_to_cafe_1": ,
        "roastery_1_ship_dark_to_cafe_2": ,
        "roastery_1_ship_light_to_cafe_2": ,
        "roastery_1_ship_dark_to_cafe_3": ,
        "roastery_1_ship_light_to_cafe_3": ,
        "roastery_2_ship_dark_to_cafe_1": ,
        "roastery_2_ship_light_to_cafe_1": ,
        "roastery_2_ship_dark_to_cafe_2": ,
        "roastery_2_ship_light_to_cafe_2": ,
        "roastery_2_ship_dark_to_cafe_3": ,
        "roastery_2_ship_light_to_cafe_3": ,
        "total_dark_coffee_received_cafe_1": ,
        "total_dark_coffee_received_cafe_2": ,
        "total_dark_coffee_received_cafe_3": ,
        "total_light_coffee_received_cafe_1": ,
        "total_light_coffee_received_cafe_2": ,
        "total_light_coffee_received_cafe_3": ,
        },
    "total_cost": ,
}
...
The domain is: A coffee production company sources beans from three suppliers, roasts them at one of two facilities into either dark or light coffee, and ships the roasted coffee to three retail locations. Each supplier has a limited capacity and each roastery can roast one unit coffee bean into one unit of dark or light coffee. The retail locations have specific demands for dark and light coffee, with no existing inventory. The company's objective is to minimize the total cost, including shipping beans, roasting, and shipping roasted coffee, while ensuring that all coffee produced meets or exceeds the demand at each retail location.
Query: {{{question}}}
You have the access to variables that summarized the original supplier, roastery, and retail information: 
capacity_in_supplier = {'supplier1': 150, 'supplier2': 50, 'supplier3': 100}
shipping_cost_from_supplier_to_roastery = {
...
def ours(index, question, llm = 'gpt-4o'):
    output_path = f'output/claude/coffee/{index}/'
        ...
        if 'Rating: 0' in self_assess_response:
            ...
            for result in result_list:
                if 'total_cost' in result:
                    total_cost = result.replace('total_cost, ', '').replace(')', '').replace(']', '').replace('[', '').replace('(', '')
...
def direct(index, question, llm = 'gpt-4o'):
    output_path = f'output/direct/coffee/{index}/'
    if not os.path.exists(output_path):
        os.makedirs(output_path)
    start = time.time()
...
def CoT(index, question, llm = 'gpt-4o'):
    output_path = f'output/CoT/coffee/{index}/'
    if not os.path.exists(output_path):
        os.makedirs(output_path)
    start = time.time()
...
def code_generation(index, question, llm = 'gpt-4o', mode = 'all'):
    if mode == 'SMT':
        output_path = f'output/code_generation_SMT/coffee/{index}/'
    else:
        output_path = f'output/code_generation/coffee/{index}/'
    if not os.path.exists(output_path):
        os.makedirs(output_path)
    start = time.time()
    if mode == 'SMT':
...
if __name__ == '__main__':
    ...
    parser.add_argument("--index_end", type=int, default=5)
    args = parser.parse_args()

    bench_path =  f'OptiGuide/benchmark/QAs/coffee.benchmark.json'
    ...
    for i in range(args.index_start, args.index_end):
        if args.test_mode == 'direct':
            ...
            if isinstance(answers[i], float):
                output = ours(i, questions[i], llm = args.llm)
                output_path = f'output/claude/coffee/{i}/'
