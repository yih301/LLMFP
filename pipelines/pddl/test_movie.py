...
The task is: 
You work to play a movie. You want to get several objects as snacks (objects could be chips, dip, pop, cheese, crackers), and have movie_rewound and set counter_at_zero at the end. You have nine actions: rewind-movie, reset-counter, start-movie, undo-rewind, get-chips, get-dip, get-pop, get-cheese, and get-crackers.
The actions defined in this domain include: 
start-movie: allows to start movie if counter_at_zero. After the start-movie action, counter_at_zero is no longer true, and counter_at_other_than_zero is true.
rewind-movie: allows to rewind movie if counter_at_other_than_zero. After the rewind-movie action, movie_rewound is true.
undo-rewind: allows to undo movie rewind if movie_rewound is true. After the undo-rewind action, movie_rewound is no longer true. 
reset-counter: allows to reset counter if counter_at_other_than_zero. After the reset-counter action, counter_at_other_than_zero is no longer true, and counter_at_zero is true. 
get-chips: allows to get an object if counter_at_zero is true, movie_rewound is not true, and this object is chips. After the get-chips action, have_chips is true.
get-dip: allows to get an object if counter_at_zero is true, movie_rewound is not true, and this object is dip. After the get-dip action, have_dip is true.
get-pop: allows to get an object if counter_at_zero is true, movie_rewound is not true, and this object is pop. After the get-pop action, have_pop is true.
get-cheese: allows to get an object if counter_at_zero is true, movie_rewound is not true, and this object is cheese. After the get-cheese action, have_cheese is true.
get-crackers: allows to get an object if counter_at_zero is true, movie_rewound is not true, and this object is crackers. After the get-crackers action, have_crackers is true.
Query: 
{{{question}}}
...
while timestep < 10:
    ...
    if solver.check() == sat: 
        ...
        for k in solver.model():
            if solver.model()[k] and ('start_movie' in str(k) or 'rewind_movie' in str(k) or 'reset_counter' in str(k) or 'undo_rewind' in str(k) or 'get_chips' in str(k) or 'get_pop' in str(k) or 'get_cheese' in str(k) or 'get_dip' in str(k) or 'get_crackers' in str(k)):
...
def code_generation(index, question, llm = 'gpt-4o', mode = 'all'):
    if mode == 'SMT':
        output_path = f'output/claude/code_generation_SMT_optimal/{task}/{index+1}/'
    else:
    ...
    start = time.time()
    if mode == 'SMT':
        code_response = Claude_response(code_gen_SMT_optimal_prompt.replace('{{{question}}}', question))
    else:
...
if __name__ == '__main__':
    ...
    task = 'movie'
    bench_path =  f'database/movie/'
    answer_path =  f'database/movie_gold_plans/'
    count = 0
    for i in range(args.index_start, args.index_end):
        if i+1 not in [5,7,9,17]:
            ...
            if args.test_mode == 'direct':
                ...
                with open(output_path+ 'answer.txt', 'w') as f:
                    f.write(str(answer))
                f.close()
            elif args.test_mode == 'verify':
                output_path = f'output/claude/CoT/{task}/{i+1}/'
                try:
                    with open(answer_path + "instance-{}_gold_plan.txt".format(i+1), 'r') as f:
                        answer = f.read()
                        print(i+1, len(answer.split('\n'))-2)
                except:
                    ...