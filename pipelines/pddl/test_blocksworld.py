...
stack: allows the arm to stack a block on top of another block if the arm is holding the top block and the bottom block is clear. After the stack action, the arm will be empty thus not holding the block, the top block will be clear and on top of the bottom block, and the bottom block will no longer be clear.
unstack: allows the arm to unstack a block from on top of another block if the top block is on the bottom block, the arm is empty, and the top block is clear. After the unstack action, the arm will be holding the top block thus not empty, the top block will no longer be on top of the bottom block and not clear, and the bottom block will be clear.
Query: 
{{{question}}}
...
The robot has four actions: pickup, putdown, stack, and unstack. The domain assumes a world where there are a set of blocks that can be stacked on top of each other, an arm that can hold one block at a time, and a table where blocks can be placed.
The actions defined in this domain include:
pickup: allows the arm to pick up a block if the block is clear, the block is on table, and the arm is empty. After the pickup action, the arm will be holding the block thus not empty, and the block will no longer be on the table or clear.
putdown: allows the arm to put down a block if the arm is holding a block. After the putdown action, the arm will be empty thus not holding the block, and the block will be on the table and clear.
stack: allows the arm to stack a block on top of another block if the arm is holding the top block and the bottom block is clear. After the stack action, the arm will be empty thus not holding the block, the top block will be clear and on top of the bottom block, and the bottom block will no longer be clear.
unstack: allows the arm to unstack a block from on top of another block if the top block is on the bottom block, the arm is empty, and the top block is clear. After the unstack action, the arm will be holding the top block thus not empty, the top block will no longer be on top of the bottom block and not clear, and the bottom block will be clear.
Query: 
{{{question}}}
...
while timestep < 18:
    ...
    if solver.check() == sat: 
        ...
        for k in solver.model():
            if solver.model()[k] and ('pickup' in str(k) or 'putdown' in str(k) or 'stack' in str(k) or 'unstack' in str(k)):
...
def code_generation(index, question, llm = 'gpt-4o'):
...
def code_generation(index, question, llm = 'gpt-4o', mode = 'all'):
    if mode == 'SMT':
        output_path = f'output/claude/code_generation_SMT_optimal/{task}/{index}/'
    else:
        output_path = f'output/claude/code_generation_optimal/{task}/{index+1}/'
    if not os.path.exists(output_path):
        os.makedirs(output_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--task", type=str, default="blocksworld_3") # 'blocksworld_3' or 'blocksworld'
    parser.add_argument("--llm", type=str, default="gpt-4o")
    parser.add_argument("--index_start", type=int, default=0)
    parser.add_argument("--index_end", type=int, default=5)
    args = parser.parse_args()

    if args.task == 'blocksworld_3':
        task = 'blocksworld_3'
        bench_path =  f'LLMs-Planning/plan-bench/instances/blocksworld/generated_basic_3/'
        answer_path =  f'LLMs-Planning/llm_planning_analysis/results/blocksworld_3/gpt-4_chat/task_1_plan_generation.json'
    else:
        task = 'blocksworld'
        bench_path =  f'LLMs-Planning/plan-bench/instances/blocksworld/generated_basic/'
        answer_path =  f'LLMs-Planning/llm_planning_analysis/results/blocksworld/gpt-4_chat/task_1_plan_generation.json'
